async function fetchUserAccessData() {
  ...
  try {
    ...
    document.getElementById('kpi-5-label').textContent = 'Unique Users';
    document.getElementById('kpi-5-val').textContent = new Set(accessRows.map(a => a.email)).size;
...
function exportAccessToXLSX() {
  if (!rawStore.access || rawStore.access.length === 0) return;
  const data = rawStore.access.map(a => ({
    "Group / Team Name": a.team,
    "Type": a.type,
    "User Display Name": a.name,
    "User Principal / Email": a.email
  }));
  exportToExcelFile({ "Access & Permissions": data }, "AzureDevOps_Security_Access");
}